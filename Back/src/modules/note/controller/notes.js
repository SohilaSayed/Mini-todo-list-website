...
async(req,res,next)=>{
    ...
    const note = await noteModel.findOneAndUpdate({_id : id , createdBy : req.user._id },{title,description,status : 'Confirmed'},{new : true})
...
async (req,res,next)=>{
    ...
    const note = await noteModel.find({createdBy : req.user._id}).populate([{
        path : 'createdBy',
        select : 'name , email , status'
    }])
    
    return note ? res.json({note}) :
